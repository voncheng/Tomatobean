...
export function tabBarDecrementAndRedirect(currentLocation) {
  ...
  return (dispatch, getState) => {
    ...
    const routingPathname = routing.locationBeforeTransitions.pathname;
    if (tabBarLocations.length > 1) {
      for (let i = 0, len = tabBarLocations.length; i < len; i++) {
        const locationPathname = tabBarLocations[i].pathname;
        if (locationPathname === currentPathname) {
          if (i < tabBarLocations.length - 1) {
            if (routingPathname === currentPathname) {
              dispatch(redirect(tabBarLocations[i + 1]));
            }
          } else if (routingPathname === currentPathname) {
            dispatch(redirect(tabBarLocations[i - 1]));
          }
        }
      }
...
export function rollBack(namespace) {
  return { type: STATE_ROLL_BACK, namespace };
}
